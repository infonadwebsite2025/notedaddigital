...
import { Menu, X, Phone, ChevronDown, Zap, Star } from 'lucide-react';
  ...
  return (
    <nav className={`w-full fixed top-0 z-50 backdrop-blur-xl transition-all duration-700 ease-out ${isScrolled
        ...
        <div className="md:hidden px-6 pb-6 bg-gradient-to-b from-white/95 to-purple-50/50 backdrop-blur-lg shadow-2xl border-t border-purple-200/30 animate-in slide-in-from-top duration-300">
          <ul className="flex flex-col gap-4 text-gray-800 text-base font-medium">
                  ...
                  <div className="pl-4 py-2 space-y-2 animate-in slide-in-from-top-2 fade-in duration-300">
                    {link.dropdownItems.map((item, subIdx) => (
                      <Link
                        key={subIdx}
                        to={routeMap[item]}
                        className="group flex items-center justify-between p-3 rounded-lg hover:bg-white/70 hover:text-purple-700 transition-all duration-300 text-sm text-gray-600 hover:scale-105 transform hover:shadow-md"
                        style={{ animationDelay: `${subIdx * 50}ms` }}
                        onClick={() => setIsMenuOpen(false)}
                      >
                        <span>{item}</span>
                        <Star className="w-4 h-4 text-purple-500 opacity-80 group-hover:opacity-100 transition-opacity duration-300" />
                      </Link>
                    ))}
                  </div>
                )}
            ...
            <button
              onMouseEnter={() => setShowPhone(true)}
              onMouseLeave={() => setShowPhone(false)}
              className="mt-6 bg-gradient-to-r from-gray-900 to-black text-white py-4 px-6 rounded-2xl font-semibold shadow-lg flex items-center justify-center gap-2 hover:shadow-2xl hover:shadow-purple-500/25 hover:scale-105 transform transition-all duration-300 group relative overflow-hidden"
            >
              <div className="absolute inset-0 bg-gradient-to-r from-purple-600 to-cyan-600 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
              <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover:translate-x-full transition-transform duration-700"></div>
              <div className="relative z-10 flex items-center gap-2">
      ...
      <style jsx>{`
        ...
        /* (Removed) mobile-only shimmer; we now mirror desktop hover behavior */
      `}</style>
    </nav>
  );
};
