...
const navLinks = [
  ...
  { name: 'Who are we?', hasDropdown: false },
...
const Navbar = () => {
  ...
  return (
    <nav
      className={`w-full fixed top-0 z-50 backdrop-blur-xl transition-all duration-700 ease-out ${
        isScrolled
              ...
              {hoveredLink === idx && link.hasDropdown && (
                <div className="absolute left-0 mt-2 w-72 bg-white/95 backdrop-blur-lg shadow-2xl rounded-2xl z-50 p-4 space-y-2 border border-purple-100/50 animate-in slide-in-from-top-5 fade-in duration-300">
                  ...
                  {link.dropdownItems.map((item, subIdx) => (
                    <div
                      key={subIdx}
                      className="relative group/item cursor-pointer p-3 rounded-xl transition-all duration-300 hover:bg-gradient-to-r hover:from-purple-50 hover:to-cyan-50 hover:scale-105 transform hover:shadow-lg"
                ...
                {link.hasDropdown && openDropdownIndex === idx && (
                  <div className="pl-4 py-2 space-y-2 animate-in slide-in-from-top-2 fade-in duration-300">
                    {link.dropdownItems.map((item, subIdx) => (
                      <div
                        key={subIdx}
                        className="p-3 rounded-lg hover:bg-white/70 hover:text-purple-700 transition-all duration-300 cursor-pointer text-sm text-gray-600 hover:scale-105 transform hover:shadow-md"
                        style={{ animationDelay: `${subIdx * 50}ms` }}
                      >
                        {item}
                      </div>
